...
@implementation BdNativeExpressViewFactory{
...
-(NSObject<FlutterPlatformView> *)createWithFrame:(CGRect)frame viewIdentifier:(int64_t)viewId arguments:(id)args{
    BdNativeExpressView * bdNativeExpressView = [[BdNativeExpressView alloc] initWithWithFrame:frame viewIdentifier:viewId arguments:args binaryMessenger:_messenger];
    return bdNativeExpressView;
}

@end


@interface BdNativeExpressView()<BaiduMobAdNativeAdDelegate>
...
@implementation BdNativeExpressView
...
- (instancetype)initWithWithFrame:(CGRect)frame viewIdentifier:(int64_t)viewId arguments:(id)args binaryMessenger:(NSObject<FlutterBinaryMessenger> *)messenger{
    if ([super init]) {
        ...
        NSString* channelName = [NSString stringWithFormat:@"com.gstory.flutter_baiduad/NativeExpressAdView_%lld", viewId];
...
//加载广告
-(void)loadNativeAd{
    GLog(@"优选信息流广告: codeId=>%@",self.codeId);
...
- (void)nativeAdObjectsSuccessLoad:(NSArray *)nativeAds nativeAd:(BaiduMobAdNative *)nativeAd{
    GLog(@"优选信息流广告: 请求成功 数量=>%lu",[nativeAds count]);
    for (int i = 0; i < nativeAds.count; i++){
        BaiduMobAdExpressNativeView *view = [nativeAds objectAtIndex:i];
        GLog(@"优选信息流广告: 请求成功 是否过期=>%d",view.isExpired);
        GLog(@"优选信息流广告: 请求成功 广告类型=>%ld",view.style_type);
...
- (void)nativeAdsFailLoadCode:(NSString *)errCode message:(NSString *)message nativeAd:(BaiduMobAdNative *)nativeAd{
    GLog(@"优选信息流广告: 请求失败 %@  %@",errCode,message);
